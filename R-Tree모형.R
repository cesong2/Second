...
# CART(Classification And Regression Trees) gini 계수가 작아지는 방향으로 움직이며 gini 계수를 가장 많이 감소시켜 주는 변수가영향력이 큰 변수가 됨
# 반복적으로 두개의 자식 노드를 생성하기 위해 모든 변수를 사용하여 데이터를 나누는 방식
# ctree : 별도로 가지치기를 할 필요가 없음, 변수의 개수는 31개로 제한됨
# C4.5, C5.0: 가장 최근의 트리 알고리즘, 샘플 개수가 적을 경우분류가 잘 안되는 단점
